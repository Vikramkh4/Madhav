...
class ComplainCategory extends BaseController
{
    ...
    {   $data['page_title'] = "Complain Category";
        ...
        return view(ACCO.'Complaincategory/add_category',$data);
    }
    ...
    {   $data['page_title'] = "Complain Category";
       ...
       $this->validation->setRules([
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ACCO.'add_complain')->with('validation', $this->validator);
        }
        ...
        if ($this->complaint->insert($data)) {
            return redirect()->to(ACCO . 'complaincategory_table')->with('success', 'Successfully Created');
        } else {
            return redirect()->to(ACCO . 'complaincategory_table')->with('error', 'Something went wrong');
        }
    ...
    public function complain_list(){
        ...
        $data['category'] = $this->complaint->findAll();
          ...
          return view(ACCO.'Complaincategory/index',$data);
    }
    ...
    if ($row !== null) {
        ...
        return view(ACCO . "Complaincategory/update_compaint", $data);
    }
    
    session()->setFlashdata('error', ' Not  found');
    return redirect()->to(ACCO . 'complaincategory_table');   
}
    ...
    {   $data['page_title'] = "Complain Category";
       ...
       $this->validation->setRules([
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ACCO.'update_companin/'.$id)->with('validation', $this->validator);
        }
        ...
        if ($this->complaint->update($id, $data)) {
            return redirect()->to(ACCO . 'complaincategory_table')->with('success', 'Successfully update');
        } else {
            return redirect()->to(ACCO
            . 'complaincategory_table')->with('error', 'Something went wrong');
        }

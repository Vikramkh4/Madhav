...
class FeesGroup extends BaseController
{
    ...
    {  
        ...
        return view(ACCO .'feesgroup/addfeesgroup',$data);
    }
    ...
    public function saving_feesgroup()
    {
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ACCO .'feesgroup')->with('validation', $this->validator)->with('page_title' ,"Fees Group");
        }
        ...
        if ($this->feesGroup->insert($data)) {
            return redirect()->to(ACCO  . 'feesgroup_table')->with('success', 'Successfully Created');
        } else {
            return redirect()->to(ACCO  . 'feesgroup_table')->with('error', 'Something went wrong');
        }
    }
...
public function feesgrouptype_list()
{
    ...
    return view(ACCO  . 'feesgroup/index', $data);
}
    ...
    public function update_feesgroupage($id){
          $data['class']  = $this->degree->findAll();
        if($id === null){
            return redirect()->to(ACCO  . 'feesgroup_table')->with('error', 'Id not found or you empty');
        }
        ...
        if($row){
            ...
            return view(ACCO ."feesgroup/updatefeesgroup",$data);
        }else{
             return redirect()->to(ACCO  . 'feesgroup_table')->with('error', 'Fial to load update page');
        }
    ...
    public function update_savegroup_fun($id){
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ACCO .'update_feesgroup')->with('validation', $this->validator)->with('page_title' ,"Update Fees Group");
        }
         ...
         if ($this->feesGroup->update($id,$data)) {
            return redirect()->to(ACCO  . 'feesgroup_table')->with('success', 'Successfully Updated');
        } else {
            return redirect()->to(ACCO  . 'feesgroup_table')->with('error', 'Fail to Update');
        } 
    }

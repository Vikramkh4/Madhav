...
class Complain extends BaseController
{
    ...
    public function index(): string {
        ...
        return view(ACCO . 'Announcement2/addcomplain', $data);
    }
    ...
    public function save_complain() {
        ...
        if ($this->complain->insert($data)) {
            return redirect()->to('accountant/complain')->with('success', 'Complain created successfully');
        } else {
            return redirect()->to('accountant/complain')->with('error', 'Failed to create complain');
        }
    }
public function complain_list() {
        ...
        return view(ACCO . 'complain/index', $data);
    }
   public function complain_sol($id){
        ...
        if ($this->complain->update($id, $data)) {
            return redirect()->to(base_url('accountant/complain'))->with('success', 'Solution saved successfully.');
        } else {
            return redirect()->to(base_url('accountant/complain'))->with('error', 'Failed to save solution.');
        }
    }
    ...
    public function edit_complain($id) {
        ...
        if (!$complain) {
            return redirect()->to('accountant/complain_table')->with('error', 'Complain not found');
        }
        ...
        return view(ACCO . 'complain/complainedit', $data);
    }
    ...
    public function update_complain($id) {
        ...
        if ($this->complain->update($id, $data)) {
            return redirect()->to('accountant/complain_table')->with('success', 'Complain updated successfully');
        } else {
            return redirect()->to('accountant/complain_table')->with('error', 'Failed to update complain');
        }
    }
    ...
    public function view_complain($id) {
        ...
        return view('accountant/complain/view_complain', $data);
    }

...
class Filename extends BaseController
{
    ...
    {  
        ...
        return view('admin/feetype/addfiletype',$data);
    }
    ...
    public function saving_filetype()
    {
        ...
        if (!$this->validate($this->validation->getRules())) {
            return view(ADMIN . 'feetype/addfiletype', ['validation' => $this->validator, 'page_title' => "Degree"]);
        }
        ...
        if ($this->filetype->insert($data)) {
            return redirect()->to(ADMIN . 'feetype_table')->with('success', 'Successfully Created');
        } else {
            return redirect()->to(ADMIN . 'feetype_table')->with('error', 'Something went wrong');
        }
    }
    ...
    public function feestype_list()
    {
        ...
        return view('admin/feetype/index', $data);
    }
    
    
    public function update_page($id){
        if($id === null){
            return redirect()->to(ADMIN . 'feetype_table')->with('error', 'Id not found or you empty');
        }
        $data['page_title'] = "Fees update";
        $row = $this->filetype->find($id);
        if($row){
            $data['user'] = $row;
            return view(ADMIN."feetype/update_fees",$data);
        }else{
             return redirect()->to(ADMIN . 'feetype_table')->with('error', 'Fial to load update page');
        }
    ...
    public function update_save_fun($id){
         ...
         if ($this->filetype->update($id,$data)) {
            ...
            return redirect()->to(ADMIN . 'feetype_table')->with('error', 'Fail to Update');
        } 
    }

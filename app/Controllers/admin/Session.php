...
class Session extends BaseController
{
    ...
    {  
        ...
        return view(ADMIN.'session/add_session',$data);
    }
    ...
    public function saving_session()
    {
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ADMIN.'session')->with('validation', $this->validator);
        }
        ...
        if ($this->session->insert($data)) {
            return redirect()->to(ADMIN . 'session_table')->with('success', 'Successfully Created');
        } else {
            return redirect()->to(ADMIN . 'session_table')->with('error', 'Something went wrong');
        }
    }
...
public function session_list()
{
    ...
    public function update_session_page($id){
        if($id === null){
            return redirect()->to(ADMIN . 'session_table')->with('error', 'Id not found or you empty');
        }
        $data['page_title'] = "Update Session";
        $row = $this->session->find($id);
        if($row){
            $data['session'] = $row;
            return view(ADMIN."session/update_session",$data);
        }else{
             return redirect()->to(ADMIN . 'session_table')->with('error', 'Fial to load update page');
        }
    ...
    public function update_savesession($id){
        ...
        if (!$this->validate($this->validation->getRules())) {
             ...
             return redirect()->to(ADMIN.'update_page/'.$id)->with('validation', $this->validator);
        }
       ...
       $data =  [
         ...
         if ($this->session->update($id,$data)) {
            return redirect()->to(ADMIN . 'session_table')->with('success', 'Successfully Updated');
        } else {
            return redirect()->to(ADMIN . 'session_table')->with('error', 'Fail to Update');
        } 
    }

                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="box-body table-responsive overflow-visible-lg">
                                <div>
                                    <table class="table table-hover alltable table-striped table-bordered example">
                                        <thead>
                                            <tr>
                                                <th>id</th>
                                        ...
                                        <tbody>
                                            <?php foreach($degreelist as $key => $row): ?>
                                                <tr> 
                                                    <td><?=($key+1)?></td>
                                                    <td><?=$row['fees_name']?></td>
                                                    <td><?=$row['fees_group']?></td>
                                                    <td><?=$row['class']?></td>
                                                    <td><?=$row['total']?></td>
                                                    <td>
                                                     <a href="<?=base_url(ADMIN.'update_feesgroup/'.$row["id"])?>" class="btn btn-primary" >Edit</a>
...
$(document).ready(function() {
    $('.delete-button').on('click', function() {
        ...
        if (confirm('Are you sure you want to delete?')) {
            $.ajax({
                url: '<?= base_url(ADMIN . "deletefeesgroup/") ?>'+id,
